        ...
        if(usr !== "" && pass !== ""){
            app.userLogin(usr,pass,function(data){
                if(data.responseHeader.status == 0 && data.response.sessionID){
                    ...
                    if(data.response.role.is_admin_role){
...
//                		window.location.href = "../www/index.html";
//                    }
//                    else if(data.response.roles[0].name === "teacher"||data.response.roles[0].name === "student" || data.response.roles[0].name==="content-admin" || data.response.roles[0].name==="content-contractor-admin"){
//                    	if(typeof(Storage) != undefined) {
//                    		readerAppHelper.setUserInfo(data.response);
//					    }
//                    	window.location.href = "../www/index.html";
  ...
  /*  	 app.signUp({
   ...
   // document.addEventListener("keydown",logPressedKeys,false);
    ...
    $(".signup-submit").off("click").on("click",function(e){
        ...
        $("#message-signup").addClass("hide");
        if(!$(this).hasClass("disabled") && !(f.find(".error").length)>0){
        	$(this).addClass("disabled").addClass("grey");
            ...
            if(!(f.find(".error").length)>0){
            	...
            	$("#createUser").removeClass("disabled").removeClass("grey");
                return false;
            }

        }else{
        	$("#createUser").addClass("disabled").addClass("grey");
            return false;
        }
...
function handleBack() {
    ...
    if($(".active-login-view").index() === 0){
    	...
    	if($(".signin-form-parent").hasClass("slideAnimation-signin-form-parent-0")){
    		readerAppHelper.logAppADSEvent('APP_ACTION',{
            	action_name:'BackButton/Tapped',
            	input_value:'',
            	screen_name:'SignIn',
    	...
    	else if($(".signin-screen-parent").hasClass("slideAnimation-signin-screen-parent-0") && !($(".landing-page").hasClass("hide"))){
    		readerAppHelper.logAppADSEvent('APP_ACTION',{
            	action_name:'BackButton/Tapped',
            	input_value:'',
            	screen_name:'SignUp',
