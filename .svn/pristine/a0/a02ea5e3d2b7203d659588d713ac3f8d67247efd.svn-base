...
//		console.log(versionInfo.version);
    ...
    window.SIM_OPN_SRV = /www.ck12.org/i.test(window.API_SERVER_NAME)?"http://interactives.ck12.org":"http://simdev.ck12.org";
    window.SIM_DWL_SRV = /www.ck12.org/i.test(window.API_SERVER_NAME)?"http://interactives.ck12.org":"http://simdev.ck12.org"
    var bodyParent = document.getElementsByTagName('body')[0],
        coverHtml, scrollX = 0,
        simData,simUpdateData, eidsData, stdData, thumbnailCollection, searchClick, scrollContainer, sideNav, sideNav2, conceptsArray = [],
        standardsArray = [],
        nameOfStandards = [],
        nameOfConcepts = [];
    ...
    function _loadUserProfile() {
        ...
        if (typeof String.prototype.startsWisth != 'function') {
    ...
    function cleanAvtar(callback){
      ...
      function onFileSystemSuccess(fileSystem) {
          fileSystem.root.getFile(
              "../../Library/NoCloud/" + "allspark/assets/images/ur_avt.png", {
                  create: true,
                  exclusive: false
              },
              function(entry) {
...
//	                	window.localStorage.removeItem(path);
    ...
    function init_cover(responseText) {
        ...
        function initInstructionVideo(e) {
            document.getElementById("InstructionVideo").ontouchmove = function(e) {                                                                                                                                                                                                             e.preventDefault();                                                                                                                                                                                                             }
...
//		cardDelete.addEventListener('click',deleteSim,false);
      ...
      moreLessSwitch.appendChild(mlBtnIcon);
        ...
        if (window.localStorage.getItem(simName)) {
            ...
            downloadedSims.push(simName);
            anySimDownloaded = true;
			if(!simUpdateData[simCount].allsparkSupport){			//check for allspark support for sim
				cardDownload.classList.add("update-available")
			}
        } else {
...
//			downloadSim();
    ...
    function openIframe(e) {
        ...
        if (!checkDeviceConnectivity()) {
            ...
            //........... Commented to open concepts in a new inAppTab...................//
          ...
          //  var refUrl = window.API_SERVER_URL + "/embed/#module=concept&handle=" + e.refValue + "&branch=physics&filters=&nochrome=true/referrer=simulation_tile";
            ...
            var refUrl = window.API_SERVER_URL+"/embed/#module=concept&handle="+e.refValue+"&filters=text,multimedia&branch=Physics&view_mode=embed&utm_source=simulations&utm_medium=web&utm_campaign=Embed&referrer=ck12simulations_app&nochrome=true";
                 cordova.InAppBrowser.open(refUrl, '_blank', 'location=no,zoom=no,enableViewportScale=no,toolbar=yes');
...
//			alert('App is to be updated');
              ...
              if (window.analytics) {
                ...
                if (simData["simulations"][simCount].simulationUrl == "") {
                    ...
                    if (simData["simulations"][simCount].simulationUrl.match(/window.match_url/i)) {
                        ...
                        var refUrl = window.SIM_OPN_SRV+"/simulations/physics/" + simName + '.zip',
    ...
    function downloadAllspark() {
        var fileURL = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream ? cordova.file.dataDirectory : cordova.file.externalDataDirectory,
            refUrl = window.SIM_DWL_SRV+"/simulations/common/allspark/1.0.8.zip",
            uri = encodeURI(refUrl),
        ...
        for (var simCount = 0; simCount < simData["simulations"].length; simCount++) {
            ...
            if (simId == simData["simulations"][simCount].name) {
            	...
            	readerAppHelper.addADSEvents('FBS_SIMULATION_DELETE',{
	                //action_name:'FBS_SIMULATION_DELETE',
	                memberID: JSON.parse(localStorage.AuthUserInfo).response.id,
	                artifactID:simData["simulations"][simCount].artifactID
            	});
