...
window.readerAppHelper = {
    ...
    localConfig:{}, //for storing some local information in a session
        init : function(){
//            requirejs(['../components/cryptojs/rollups/aes'], function(CryptoJs){
//                      requirejs(['../components/cryptojs/components/mode-cfb'], function(){
//
//                                });
//                      });
...
//
//                    if(readerAppHelper.checkForServer()) {
//                    	if(window._platform === 'ios' ){
//                        	// Custom returnTo url to trigger a deepLink from SafariViewController.
//                        	var authUrl = AUTH_SERVER_URL + '/auth/login/member/google?returnTo=' + escape(AUTH_SERVER_URL + '/auth/verify/member/app/google/'+readerAppHelper.deepLinkAppCode)+"&popup=false";
...
//                                });
//                        } else {
//		                    var authUrl = window.API_SERVER_URL + '/auth/login/member/google?returnTo='+ escape(window.API_SERVER_URL + '/auth/verify/member/google');
//
...
decryptData: function(ciphertext) {
    ...
    encryptData: function(string) {
        ...
        var encrypted = CryptoJS.AES.encrypt(string, secretkey, {
            iv: iv,
            mode: CryptoJS.mode.CFB
        });
